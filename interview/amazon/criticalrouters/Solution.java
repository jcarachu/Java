...
class Solution {
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println(getCritialNodes(links1, numLinks1, numRouters1));
    }

    private static List<Integer> getCritialNodes(int[][] links, int numLinks, int numRouters)
    {
        time = 0;
    ...
    private static void dfs(Map<Integer, Set<Integer>> map, int[] low, int[] ids, int[] parent, int cur, Set<Integer> res)
    {
        ...
        for (int nei: map.get(cur))
        {
            if (ids[nei] == -1)
            {
                ...
                if((parent[cur] == -1 && childern > 1) || (parent[cur] != -1 && low[nei] >= ids[cur]))
                    res.add(cur);
